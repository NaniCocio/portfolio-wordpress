...
var arrival_customize_control_tabs = function ( $ ) {
	'use strict';
		...
		function () {
			...
			$( '.arrival-tabs-control' ).each(
				function () {
					...
					var shownCtrls = $( this ).find( '.arrival-customizer-tab > input:checked' ).data( 'controls' );
					showControls( customizerSection, shownCtrls );
				}
			);

			$( '.arrival-customizer-tab > label' ).on(
				'click', function () {
					var customizerSection = $( this ).closest( '.accordion-section' );
...
arrival_customize_control_tabs( jQuery );
