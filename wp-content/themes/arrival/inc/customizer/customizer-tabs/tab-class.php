 ...
 * @access public
 */
class Arrival_Customize_Control_Tabs extends WP_Customize_Control {

	/**
	 * Arrival_Customize_Control_Tabs constructor.
	 *
	...
	public function render_content() {
		...
		$output .= '<div class="arrival-tabs-control" id="input_' . esc_attr( $this->id ) . '">';
		foreach ( $this->tabs as $value => $args ) {
			if ( ! empty( $args['controls'] ) && ( $this->tab_has_controls( $args['controls'] ) ) ) {
				$controls_attribute = json_encode( $args['controls'] );

				$output .= '<div class="arrival-customizer-tab">';
				...
				$output .= '</label>';
				$output .= '</div><!-- /.arrival-customizer-tab -->';
			}
		}
		$output .= '</div><!-- /.arrival-tabs-control -->';

		echo $output; //sanitization already done
	}
	/**
	...
	public function enqueue() {
		...
		wp_enqueue_script( 'arrival-tabs-control-script', get_template_directory_uri() . '/inc/customizer/customizer-tabs/js/script.js', array( 'jquery' ), '15258', true );
		wp_enqueue_style( 'arrival-tabs-control-style', get_template_directory_uri() . '/inc/customizer/customizer-tabs/css/style.css', null, '15258' );

	}

	/**
	 * Enqueue the partials handler script that works synchronously with the arrival-tabs-control-script
	 */
	public function partials_helper_script_enqueue() {
		wp_enqueue_script( 'arrival-tabs-addon-script', get_template_directory_uri() . '/inc/customizer/customizer-tabs/js/addons.js', array( 'jquery' ), '15258', true );
	}
